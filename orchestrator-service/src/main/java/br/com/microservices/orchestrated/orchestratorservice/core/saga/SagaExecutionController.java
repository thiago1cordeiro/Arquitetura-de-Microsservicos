...
public class SagaExecutionController {
    ...
    private ETopics findTopicBySourceAndStatus(Event event) {
        return (ETopics) (Arrays.stream(SAGA_HANDLER)
                .filter(row -> isEventSourceAndStatusEvent(event, row))
                .map(i -> i[TOPIC_INDEX])
                .findFirst()
    ...
    private boolean isEventSourceAndStatusEvent(Event event, Object[] row) {
    ...
    private String createSagaId(Event event) {
        return format(SAGA_LOG_ID,
                event.getPayload().getId(), event.getTractionId(), event.getId());
    }
}
